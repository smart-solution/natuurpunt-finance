...
class res_partner(osv.osv):
    ...
    _columns = {
        'property_supplier_cost_account': fields.property(
            'account.account',
            type='many2one',
            relation='account.account',
            string="Default Cost Account",
            view_load=True,
            domain="[('type', '=', 'other')]",
...
class account_invoice_line(osv.osv):
    ...
    def default_get(self, cr, uid, fields, context=None):
        """Get the cost account from the supplier"""
        res = super(account_invoice_line, self).default_get(cr, uid, fields, context=context)
        if 'type' in context and context['type'] and context['type'] == 'in_invoice':
            if 'partner_id' in context and context['partner_id']:
                partner = self.pool.get('res.partner').browse(cr, uid, context['partner_id'])
                if partner and partner.property_supplier_cost_account:
                    res['account_id'] = partner.property_supplier_cost_account.id
        return res
