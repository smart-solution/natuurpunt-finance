        ...
        if reference_type == 'bba':
            ...
            if self.check_bbacomm(reference):
                ...
                same_ids = self.search(cr, uid,
                    [('type', '=', 'out_invoice'), ('reference_type', '=', 'bba'),
                     ('reference', '=', vals['reference']), ('company_id','=',user.company_id.id)])
                if same_ids:
                    same_inv = self.browse(cr, uid, same_ids[0])
                    raise osv.except_osv(_('Warning!'),
                        _('The BBA Structured Communication %s has already been used for this company!' \
                          '\nPlease create manually a unique BBA Structured Communication.')%(vals['reference'], same_inv.number))
                ...
                if self.check_bbacomm(bbacomm):
                    ...
                    same_ids = self.search(cr, uid,
                        [('id', '!=', inv.id), ('type', '=', 'out_invoice'),
                         ('reference_type', '=', 'bba'), ('reference', '=', vals['reference']), ('company_id','=',inv.company_id.id)])
                    if same_ids:
                        same_inv = self.browse(cr, uid, same_ids[0])
                        raise osv.except_osv(_('Warning!'),
                            _('The BBA Structured Communication %s has already been used for this company!' \
                                    '\nPlease create manually a unique BBA Structured Communication.\nInvoice: %s')%(vals['reference'], same_inv.number))
